use iced::{alignment::Vertical, Element, Length::{self, Fill}};
...
impl Application {
    ...
    pub fn view(&self) -> Element<'_, Message> {
        ...
        let (omni_button_label, omni_button_message) = match self.submenu {
            SubMenu::SelectProfile => ("Configure Profile", Some(Message::ButtonSettingsPressed)),
            SubMenu::ConfigureProfile => ("Back", Some(Message::ButtonBackPressed)),
            SubMenu::Settings => ("Back", Some(Message::ButtonBackPressed)),
        };
        let settings_button = iced::widget::button(omni_button_label).on_press_maybe(omni_button_message);
    ...
    fn view_select_profile(&self) -> Element<'_, Message> {
        let profile_to_widget = |(i, p)| -> (usize, Element<'_, Message>) {
            let text_color = self.selected_profile.is_some_and(|p_i| p_i == i).then(|| iced::Color::from_rgba(0.3, 0.7, 0.2, 1.0)); 
            let text = iced::widget::text!("{}", p).width(Fill).color_maybe(text_color);
            (
                i, 
                iced::widget::mouse_area(text).on_press(Message::SelectProfile(i)).into()
            )
        };
        let profiles = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K"];
        let profiles = iced::widget::keyed_column(profiles.iter().enumerate().map(profile_to_widget))
            .width(Length::Fill)
